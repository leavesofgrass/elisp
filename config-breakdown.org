...
#+BEGIN_SRC bash :tangle yes
sudo apt install git sox tclx8.6-dev -y && sudo apt-get build-dep emacs25 emacspeak -y
#+END_SRC
...
#+BEGIN_SRC bash :tangle yes
sed -e "\$aexport DTK_PROGRAM" ~/.bashrc > ~/.bashrc 

sed -e "\$aDTK_PROGRAM=espeak" ~/.bashrc > ~/.bashrc 
#+END_SRC
